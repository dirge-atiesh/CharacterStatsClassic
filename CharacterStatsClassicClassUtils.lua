...
function CSC_GetBonusUndeadSpellDamage(unit)

   local bonusUndeadSpellDamage = 0;
   local firstItemslotIndex = 1;
   local lastItemslotIndex = 18;
   ...
   for itemSlot = firstItemslotIndex, lastItemslotIndex do
      local itemId = GetInventoryItemID(unit, itemSlot);
      if (itemId) then
         local bonus = g_UDCItemIdToSP[itemId];
         if(bonus) then
            bonusUndeadSpellDamage = bonusUndeadSpellDamage + bonus;
         end
      end
   end

   _, _, _, enchantID, _, _, _, _ = GetWeaponEnchantInfo()
   if (enchantID == 2685) then
      bonusUndeadSpellDamage = bonusUndeadSpellDamage + 60;
   end

   return bonusUndeadSpellDamage;
end
